...
func (s *Service) Dispatch(ctx context.Context, principal domain.Principal, caseID, requestKey string, expectedVersion int64) ([]domain.Inquiry, error) {
	...
	jobs := make([]store.Job, 0, len(institutions))
	for _, institutionID := range institutions {
		...
		created = append(created, domain.Inquiry{ID: inquiryID, CaseID: caseID, InstitutionID: institutionID,
		...
		jobs = append(jobs, store.Job{ID: jobID, Kind: "dispatch_inquiry", ResourceID: inquiryID,
